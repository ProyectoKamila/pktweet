...
//            $this->load->view('inicio', $this->data);
            ...
            if (isset($_SESSION['status']) && $_SESSION['status'] == 'verified') { //Success, redirected back from process.php with varified status.
                ...
                $this->data['estatuslista'] = $this->modelo_universal->select('lista_ciclica', 'estatus,intervalo', array('id_cuenta' => $iden));
...
//                $this->load->view('authorize', $this->data);
    ...
    public function retweet($id_cuenta = null) {
        if ($this->user->conect && $id_cuenta <> null) {
            $this->data['retweet'] = $this->modelo_universal->select('retweet', '*', array('id_cuenta' => $id_cuenta, 'id_account' => $this->user->information->id));
        }
    }
...
//                        debug($this->db->last_query());
    ...
    public function deleteretweet() {
        if ($this->user->conect) {
            $this->modelo_universal->delete('retweet', array('retweet_cuenta' => $this->input->post('d'), 'id_cuenta' => $this->input->post('id'), 'id_account' => $this->user->information->id));
            $this->data['iden'] = $this->input->post('id');
            $this->retweet($this->input->post('id'));
            $this->load->view('kapp-tweet/retweet');
        }
        ...
        for ($rate = 0; $rate <= $number; $rate++) {
            ...
            if ($string) {
                ...
                foreach ($user as $u) {
                    ...
                    if (is_object($u) && $u->connections[0] != 'following') {
...
//        $this->load->view('listas', $this->data); //se carga la vista con todos los datos recogidos
    ...
    public function elimdirect() {
        if ($this->user->conect) {
            $this->modelo_universal->delete('directmessage', array('id' => $this->input->post('id'), 'idaccount' => $this->user->information->id));
        }
    }
    ...
    public function actualizar($id) {
        ...
        $this->data['ctas'] = $this->modelo_universal->select('cuentas', '*', array('id_account' => $idaccount));
        $this->data['widget'] = $this->modelo_universal->select('widget', '*', array('id_account' => $idaccount));
        if (isset($this->data['ctas'][0]['id_cuenta'])) {
            $idtwet = $this->data['ctas'][0]['id_cuenta'];
...
//    debug($id);
    ...
    public function followusersearch() {
        $idaccount = $this->input->post('idaccount');
        $id = $this->input->post('id');
        if ($this->user->conect) {
            ...
            if ($user) {
                ...
                $url = 'https://api.twitter.com/1.1/friendships/create.json';
                $results = $twitteroauth->post($url, array('user_id' => $id));
                $cpseguidores = $user[0]['cpseguidores'] - 1;
    ...
    public function listdirectmessage($id = null) {
        if ($this->user->conect) {
            $this->data['directmessage'] = $this->modelo_universal->select('directmessage', 'id,message', array('id_cuenta' => $id, 'idaccount' => $this->user->information->id), null, null, 'id', 'DESC');
        }
    }

    public function directmessage() {
        if ($this->user->conect) {
            $check = $this->modelo_universal->select('cuentas', 'id_cuenta', array('id_cuenta' => $this->input->post('id'), 'id_account' => $this->user->information->id));
    ...
    public function cupon($cuenta) {
        if ($this->user->conect) {
            if ($this->user->information->pktweet > 0) {
                ...
                $date = $this->modelo_universal->select('cuentas', 'fecha_f', array('id_cuenta' => $cuenta, 'id_account' => $this->user->information->id, 'fecha_f !=' => '0000-00-00', 'fecha_f >' => $today));
                if ($date) {
                    $nuevafecha = strtotime('+31 day', strtotime($date[0]['fecha_f']));
                    $nuevafecha = date('Y-m-d', $nuevafecha);
                    $date = $this->modelo_universal->update('cuentas', array('fecha_f' => $nuevafecha), array('id_cuenta' => $cuenta, 'id_account' => $this->user->information->id));
                } else {
                    $today = date('Y-m-d');
